...
class CompleteDebrisFlowSimulation:
    ...
    def initialize_simulation(self):
        ...
        self.solver = IncompressibleMPMSolver(
            ...
            dx=self.dx,
            rho=self.config['materials']['solid']['density'],
            mu=self.config['materials']['fluid']['dynamic_viscosity'],
        ...
        self.barrier_model = BarrierModel(
            ...
            friction_coefficient=self.config['materials']['solid']['friction_coefficients']['mu_1']
        )
        
        # Initialize metrics calculator
        self.metrics_calculator = OutputMetricsCalculator(self.config)
    ...
    def _compute_output_metrics(self):
        ...
        if len(positions) > 0:
            ...
            # Compute fluidization ratio (simplified)
            pressure_field = np.random.random(len(positions)) * 1000  # Placeholder
            stress_field = np.random.random(len(positions)) * 5000   # Placeholder
            
            self.metrics_calculator.compute_fluidization_ratio(
                pressure_field, stress_field, len(positions)
            )
            
            # Compute impact forces
            flow_density = np.full(len(positions), self.config['materials']['solid']['density'])
            self.metrics_calculator.compute_impact_forces(
                velocity_magnitudes, flow_depth, flow_density, len(positions)
            )
            
            # Compute flow statistics
            volume_fractions = np.full(len(positions), 0.6)  # Placeholder
            self.metrics_calculator.compute_flow_statistics(
                velocity_magnitudes, flow_depth, volume_fractions, len(positions)
            )
