...
# matplotlib disabled for server compatibility
...
class CompleteDebrisFlowSimulation:
    ...
    def __init__(self, config_path: str = "physics_config.yaml", config_override: dict = None):
        ...
        if config_override:
            for category, settings in config_override.items():
        ...
        # Initialize Taichi
        # Use positional args for older Taichi compatibility
        ...
        self.ny = int(self.domain_height / self.dx)  # y = height (gravity direction)
        self.nz = int(self.domain_width / self.dx)   # z = width (out-of-plane)
    ...
    def initialize_simulation(self):
        ...
        self.solver = IncompressibleMPMSolver(
            ...
            self.dx,
            self.config['solid_phase']['density'],
            self.config['fluid_phase']['viscosity'],
            self.config['simulation']['gravity'],
            self.dt
        )

        # Initialize barrier model with slope angle
        self.barrier_model = BarrierModel(
            self.config['simulation']['barrier_height'],
            self.config['simulation']['barrier_spacing'],
            tuple(self.config['simulation']['barrier_positions']),
            self.config['numerics']['contact_stiffness'],
            self.config['numerics']['contact_damping'],
            self.config['solid_phase']['static_friction'],
            self.config['simulation']['slope_angle']  # 20° slope
        )

        # Initialize metrics calculator
        self.metrics_calculator = OutputMetricsCalculator("physics_config.yaml")

        # Initialize particles and level set with same geometry
    ...
    def _initialize_particles(self):
        """Initialize debris flow particles and level set."""
        # Debris flow initial geometry (x=length, y=height, z=width)
        # Calculate debris length from volume: V = L * H * W
        ...
        self.solver.initialize_particles_dam_break(
            0.0, debris_length,
            0.0, debris_height,
            0.0, self.domain_width,
            self.config['numerics']['particles_per_cell']
        )

        self.total_particles = self.solver.n_particles[None]
        print(f"Initialized {self.total_particles} particles")
        
        # Initialize level set to match debris flow region ONLY
        # (not the entire domain - this was incorrect before)
        self.solver.level_set_method.initialize_box(
            x_min=0.0, x_max=debris_length,
            y_min=0.0, y_max=debris_height,
            z_min=0.0, z_max=self.domain_width
        )
        print(f"Level set initialized for debris region: {debris_length:.2f}m x {debris_height:.2f}m x {self.domain_width:.2f}m")
    ...
    def _compute_output_metrics(self):
        ...
        if len(positions) > 0:
            ...
            flow_depth = np.max(positions[:, 1]) - np.min(positions[:, 1])  # y = height

            # Physics-based fluidization ratio calculation (Eq. 26 in paper)
            # λ = p_bed / (p_bed + σ'_bed) where p_bed is pore pressure, σ'_bed is effective stress
            
            # Get physical parameters
            rho_s = self.config['solid_phase']['density']      # 2650 kg/m³
            rho_f = self.config['fluid_phase']['density']      # 1000 kg/m³
            phi_s = self.config['solid_phase']['critical_solid_fraction']  # 0.56
            g = self.config['simulation']['gravity']           # 9.81 m/s²
            
            # Calculate height above each particle (for hydrostatic approximation)
            max_y = np.max(positions[:, 1])
            h_above = max_y - positions[:, 1]  # Height of material above each particle
            h_above = np.maximum(h_above, 0.01)  # Minimum height to avoid division by zero
            
            # Pore fluid pressure: p_f = ρ_f * g * h (hydrostatic in fluid)
            pore_pressure = rho_f * g * h_above
            
            # Total normal stress at depth: σ_total = (φ*ρ_s + (1-φ)*ρ_f) * g * h
            mixture_density = phi_s * rho_s + (1 - phi_s) * rho_f
            total_stress = mixture_density * g * h_above
            
            # Effective stress: σ' = σ_total - p_f = φ*(ρ_s - ρ_f) * g * h
            effective_stress = phi_s * (rho_s - rho_f) * g * h_above
            effective_stress = np.maximum(effective_stress, 1.0)  # Minimum to avoid division by zero
            
            # Fluidization ratio: λ = p_f / (p_f + σ') = p_f / σ_total
            fluidization_ratios = pore_pressure / (pore_pressure + effective_stress)
            ...
            self.metrics_calculator.solid_volume_fraction[None] = phi_s  # From config
    ...
    def _generate_analysis_plots(self, output_dir: str):
        """Generate analysis plots (disabled - no matplotlib)."""
        print("Analysis plots generation skipped (matplotlib disabled for server compatibility)")
    ...
    def _analyze_barrier_spacing_results(self, results: Dict[str, Any], output_dir: str):
        ...
        # Analysis plotting disabled for server compatibility
        print("Barrier spacing analysis plots skipped (matplotlib disabled)")
...
def main():
    ...
    # Smaller config to fit in single GPU memory
    config_override = {
        'numerics': {
            'mesh_barrier_ratio': 0.05, # Coarser grid
            'particles_per_cell': 2, # Fewer particles
        },
        'simulation': {
            'total_time': 2.0 # Shorter simulation
        }
    }
    
    # Create simulation with smaller config
    simulation = CompleteDebrisFlowSimulation(config_override=config_override)
