...
class IncompressibleMPMSolver:
    def __init__(self,
                 ...
                 preconditioner='jacobi'  # PCG preconditioner: 'jacobi', 'mic', 'ssor'
                 ):
        ...
        # Initialize PCG solver with selected preconditioner
        self.pcg_solver = PCGSolver(nx, ny, nz, dx, preconditioner=preconditioner)
    ...
    def compute_velocity_divergence_fd(self):
        ...
        for i, j, k in self.div_v_star:
            ...
            if j == 0:
                div_y = (self.grid_v_star[i, j+1, k][1] - self.grid_v_star[i, j, k][1]) / self.dx
            elif j == self.ny - 1:
                div_y = (self.grid_v_star[i, j, k][1] - self.grid_v_star[i, j-1, k][1]) / self.dx
            else:
                div_y = (self.grid_v_star[i, j+1, k][1] - self.grid_v_star[i, j-1, k][1]) / (2.0 * self.dx)
            
            # Z-direction divergence
            if k == 0:
                div_z = (self.grid_v_star[i, j, k+1][2] - self.grid_v_star[i, j, k][2]) / self.dx
            elif k == self.nz - 1:
                div_z = (self.grid_v_star[i, j, k][2] - self.grid_v_star[i, j, k-1][2]) / self.dx
            else:
    ...
    def pressure_correction_legacy(self):
        ...
        for i, j, k in self.grid_v:
            if self.grid_m[i, j, k] > 0:
                ...
                if i == 0:
                    grad_p_x = (self.pcg_solver.pressure[i+1, j, k] - self.pcg_solver.pressure[i, j, k]) / self.dx
                elif i == self.nx - 1:
                    grad_p_x = (self.pcg_solver.pressure[i, j, k] - self.pcg_solver.pressure[i-1, j, k]) / self.dx
                else:
                    grad_p_x = (self.pcg_solver.pressure[i+1, j, k] - self.pcg_solver.pressure[i-1, j, k]) / (2.0 * self.dx)
                
                # Y-direction gradient
                if j == 0:
                    grad_p_y = (self.pcg_solver.pressure[i, j+1, k] - self.pcg_solver.pressure[i, j, k]) / self.dx
                elif j == self.ny - 1:
                    grad_p_y = (self.pcg_solver.pressure[i, j, k] - self.pcg_solver.pressure[i, j-1, k]) / self.dx
                else:
                    grad_p_y = (self.pcg_solver.pressure[i, j+1, k] - self.pcg_solver.pressure[i, j-1, k]) / (2.0 * self.dx)
                
                # Z-direction gradient
                if k == 0:
                    grad_p_z = (self.pcg_solver.pressure[i, j, k+1] - self.pcg_solver.pressure[i, j, k]) / self.dx
                elif k == self.nz - 1:
                    grad_p_z = (self.pcg_solver.pressure[i, j, k] - self.pcg_solver.pressure[i, j, k-1]) / self.dx
                else:
    ...
    def step(self, pcg_max_iter=200, pcg_tol=1e-3):
        """Perform one simulation step following iMPM algorithm
        
        Args:
            pcg_max_iter: Maximum PCG iterations (default 200)
            pcg_tol: PCG convergence tolerance (default 1e-3, relaxed for stability)
        """
        ...
        iterations = self.solve_pressure_pcg(max_iter=pcg_max_iter, tol=pcg_tol)
        ...
        # Reinitialize level set periodically (every 50 steps for stability)
