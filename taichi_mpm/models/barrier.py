...
class BarrierModel:
    ...
    def __init__(self,
        ...
        Args:
            ...
            slope_angle: Channel slope angle in degrees (default 20°)
        """
        ...
        # Slope angle (convert to radians) - stored as Taichi fields for kernel access
        self.slope_angle_deg = slope_angle
        slope_rad = slope_angle * np.pi / 180.0
        self.slope_angle_rad = slope_rad
        self.cos_theta = np.cos(slope_rad)
        self.sin_theta = np.sin(slope_rad)
        
        # Taichi scalar fields for kernel access
        self._slope_rad = ti.field(ti.f64, shape=())
        self._cos_theta = ti.field(ti.f64, shape=())
        self._sin_theta = ti.field(ti.f64, shape=())
        self._slope_rad[None] = slope_rad
        self._cos_theta[None] = self.cos_theta
        self._sin_theta[None] = self.sin_theta
    ...
    def detect_contacts(self,
        ...
        Note: Barriers are perpendicular to the sloped channel bed.
        In the tilted coordinate system (x along slope, y perpendicular to slope),
        barriers act as vertical walls relative to the slope.
        """
        ...
        for i in range(n_particles):
            ...
            for barrier_id in range(2):
                ...
                # Barrier is perpendicular to the slope at x = barrier_pos.x
                # Height is measured perpendicular to the slope (y direction)
                penetration = barrier_pos.x - pos.x

                # Check if within barrier height (y is perpendicular to slope)
        ...
        for i in range(n_particles):
            ...
            for barrier_id in range(2):
                ...
                if (pos.x > barrier_pos.x and
                    ...
                    # Use configured slope angle from Taichi fields
                    cos_theta = self._cos_theta[None]
                    sin_theta = self._sin_theta[None]
                    tan_theta = sin_theta / cos_theta  # tan = sin/cos
